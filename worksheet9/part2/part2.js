...
window.onload = function init() {
    ...
    image.onload = function() {
        ...
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, text);
        gl.uniform1i(programGround.texture, 0);
...
function render() {
    ...
    l[0] = lightAnimationBool ? 2 * Math.sin(time) : -0.1;
    l[2] = lightAnimationBool ? -2 + 2 * Math.cos(time) : -2;
    ...
    var projection = mult(
        ...
        lookAt(vec3(l[0], l[1], l[2]), vec3(0, 0, -3), vec3(0, 1, 0)),
    );
