...
window.onload = function init() {
    ...
    var filteringModeDropDown = document.getElementById("dropdown");
    var wrappingModeDropDown = document.getElementById("dropdown1");
    var filteringMode = gl.NEAREST;
    var wrappingMode = gl.REPEAT;

    filteringModeDropDown.onchange = function() {
        if (filteringModeDropDown.selectedIndex == 0) {
            filteringMode = gl.NEAREST;


        }

        if (filteringModeDropDown.selectedIndex == 1) {
            filteringMode = gl.LINEAR;

        }

        if (filteringModeDropDown.selectedIndex == 2) {
            gl.generateMipmap(gl.TEXTURE_2D);
            filteringMode = gl.LINEAR_MIPMAP_NEAREST;

        }


        if (filteringModeDropDown.selectedIndex == 3) {
            gl.generateMipmap(gl.TEXTURE_2D);
            filteringMode = gl.LINEAR_MIPMAP_LINEAR;
    ...
    wrappingModeDropDown.onchange = function() {
        if (wrappingModeDropDown.selectedIndex == 0) {
            wrappingMode = gl.REPEAT;
        }

        if (wrappingModeDropDown.selectedIndex == 1) {
    ...
    // console.log("index", wrappingModeDropDown.selectedIndex);
    // console.log("filteringMode", filteringMode);
